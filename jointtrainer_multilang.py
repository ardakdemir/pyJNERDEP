...
class JointTrainer:
    ...
    def getdatasets(self):
        ...
        if self.args['mode'] == 'predict':
            self.nervalreader = DataReader(ner_test_name,"NER", batch_size = self.args['batch_size'],tokenizer=self.bert_tokenizer)
            self.depvaldataset  = DepDataset(dep_test_name, batch_size = self.args['batch_size'], vocabs = self.deptraindataset.vocabs, for_eval=True,tokenizer=self.bert_tokenizer)
        else:
            self.nervalreader = DataReader(ner_dev_name,"NER", batch_size = self.args['batch_size'], tokenizer=self.bert_tokenizer)
            self.depvaldataset  = DepDataset(dep_dev_name, batch_size = self.args['batch_size'], vocabs = self.deptraindataset.vocabs, for_eval=True,tokenizer=self.bert_tokenizer)
