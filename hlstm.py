...
class HighwayLSTM(nn.Module):
    ...
    def __init__(self, input_size, hidden_size,
                 ...
                 dropout=0, bidirectional=True, rec_dropout=0, highway_func=None, pad=False):
        ...
        for l in range(num_layers):
            self.lstm.append(nn.LSTM(in_size, hidden_size, num_layers=1,
...
if __name__ == "__main__":
    ...
    rnn = HighwayLSTM(200, 40, num_layers=2, bidirectional=True,batch_first=True)
    input = torch.randn(T, 6, 200)
    hx = torch.randn(2 * num_dir, 3, 20)
    cx = torch.randn(2 * num_dir, 3, 20)
    sent_lens = [6] * T
    output, _ = rnn(input, sent_lens)
