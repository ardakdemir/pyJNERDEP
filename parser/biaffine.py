...
class PairwiseBilinear(nn.Module):
    ...
    def __init__(self, input1_size, input2_size, output_size, bias=True):
        ...
        self.weight = nn.Parameter(torch.randn(input1_size, input2_size, output_size))
        self.bias = nn.Parameter(torch.randn(output_size)) if bias else 0
...
if __name__ == "__main__":
    x1 = torch.randn(3,4).view(1,3,4)
    x2 = torch.randn(3,5).view(1,3,5)
    scorer = DeepBiaffineScorer(4, 5, 6, 7)
    print(scorer(x1, x2).shape)
