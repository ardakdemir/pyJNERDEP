...
do
    ...
    do
        ...
        singularity exec --nv ~/singularity/pt-cuda-tf-tr-ft python jointtrainer_multilang.py --model_type ${model_type}  --word_embed_type ${type}    --log_file ${model_type}_log_${type}_${lang} --lang ${wvec_pref} --batch_size 300 --epochs 10 --save_dir $load_dir --word_only
#        if [ ! ${model_type} == 'NER' ]
#        then
