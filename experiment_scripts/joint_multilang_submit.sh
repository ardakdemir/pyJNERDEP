...
#    for type in fastext word2vec random_init
    ...
    do
        ...
        singularity exec --nv  --writable ~/singularity/pt-cuda-tf-tr-ft python jointtrainer_multilang.py --model_type ${model_type}  --word_embed_type ${type}    --log_file ${model_type}_log_${type}_${lang}_log.txt --lang ${wvec_pref}  --epochs ${epoch} --save_dir $load_dir --repeat ${repeat} --eval_interval ${eval_interval} --word_only
#        if [ ! ${model_type} == 'NER' ]
#        then
